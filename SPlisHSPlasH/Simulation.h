...
#define forall_fluid_neighbors(code)                                           \
  for (unsigned int pid = 0; pid < nFluids; pid++) {                           \
    ...
    for (unsigned int j = 0;                                                   \
         j < sim->numberOfNeighbors(fluidModelIndex, pid, i); j++) {           \
      const unsigned int neighborIndex =                                       \
          sim->getNeighbor(fluidModelIndex, pid, i, j);                        \
 ...
 * Simulation *sim, unsigned int fluidModelIndex and FluidModel* model must be
 * defined.
 */
#define forall_fluid_neighbors_in_same_phase(code)                             \
  for (unsigned int j = 0;                                                     \
       j < sim->numberOfNeighbors(fluidModelIndex, fluidModelIndex, i); j++) { \
    const unsigned int neighborIndex =                                         \
...
#define forall_boundary_neighbors(code)                                        \
  for (unsigned int pid = nFluids; pid < sim->numberOfPointSets(); pid++) {    \
    BoundaryModel_Akinci2012 *bm_neighbor =                                    \
        static_cast<BoundaryModel_Akinci2012 *>(                               \
            sim->getBoundaryModelFromPointSet(pid));                           \
    for (unsigned int j = 0;                                                   \
         j < sim->numberOfNeighbors(fluidModelIndex, pid, i); j++) {           \
      const unsigned int neighborIndex =                                       \
          sim->getNeighbor(fluidModelIndex, pid, i, j);                        \
 ...
 * Simulation *sim, unsigned int nBoundaries and unsigned int fluidModelIndex
 * must be defined.
 */
#define forall_density_maps(code)                                              \
  for (unsigned int pid = 0; pid < nBoundaries; pid++) {                       \
    ...
    if (rho != 0.0) {                                                          \
      const Vector3r &gradRho =                                                \
          bm_neighbor->getBoundaryDensityGradient(fluidModelIndex, i)          \
              .cast<Real>();                                                   \
...
#define forall_volume_maps(code)                                               \
  for (unsigned int pid = 0; pid < nBoundaries; pid++) {                       \
    BoundaryModel_Bender2019 *bm_neighbor =                                    \
        static_cast<BoundaryModel_Bender2019 *>(sim->getBoundaryModel(pid));   \
...
#define forall_fluid_neighbors_in_same_phase_avx(code)                         \
  ...
  for (unsigned int j = 0; j < maxN; j += 8) {                                 \
    const unsigned int count = std::min(maxN - j, 8u);                         \
    const Vector3f8 xj_avx = convertVec_zero(                                  \
        &sim->getNeighborList(fluidModelIndex, fluidModelIndex, i)[j],         \
...
enum class BoundaryHandlingMethods {
  Akinci2012 = 0,
  Koschier2017,
  Bender2019,
  NumSimulationMethods
  ...
  struct FluidInfo {
    ...
    bool hasSameParticleSampling(const FluidInfo &other) {
      ...
      if ((type == 1) && (other.type == 1) && (scale == other.scale)) {
        ...
        if (((box.max() - box.min())
                 .isApprox(other.box.max() - other.box.min()),
             1.0e-9) &&
            (mode == other.mode))
          return true;
      }
...
public:
  ...
  void setBoundaryHandlingMethod(BoundaryHandlingMethods val) {
    m_boundaryHandlingMethod = (int)val;
  }
  ...
  int getSimulationMethod() const {
    return static_cast<int>(m_simulationMethod);
  }
  ...
  std::vector<NonPressureForceMethod> &getDragMethods() {
    return m_dragMethods;
  }

  void addElasticityMethod(
      const std::string &name,
      const std::function<NonPressureForceBase *(FluidModel *)> &creator) {
    m_elasticityMethods.push_back({name, creator, -1});
  }
  std::vector<NonPressureForceMethod> &getElasticityMethods() {
    return m_elasticityMethods;
  }

  void addSurfaceTensionMethod(
      const std::string &name,
      const std::function<NonPressureForceBase *(FluidModel *)> &creator) {
    m_surfaceTensionMethods.push_back({name, creator, -1});
  }
  std::vector<NonPressureForceMethod> &getSurfaceTensionMethods() {
    return m_surfaceTensionMethods;
  }

  void addViscosityMethod(
      const std::string &name,
      const std::function<NonPressureForceBase *(FluidModel *)> &creator) {
    m_viscoMethods.push_back({name, creator, -1});
  }
  std::vector<NonPressureForceMethod> &getViscosityMethods() {
    return m_viscoMethods;
  }

  void addVorticityMethod(
      const std::string &name,
      const std::function<NonPressureForceBase *(FluidModel *)> &creator) {
    m_vorticityMethods.push_back({name, creator, -1});
  }
  std::vector<NonPressureForceMethod> &getVorticityMethods() {
    return m_vorticityMethods;
  }
  ...
  numberOfNeighbors(const unsigned int pointSetIndex,
                    const unsigned int neighborPointSetIndex,
                    const unsigned int index) const {
    return static_cast<unsigned int>(
        m_neighborhoodSearch->point_set(pointSetIndex)
            .n_neighbors(neighborPointSetIndex, index));
  }

  FORCE_INLINE unsigned int
  getNeighbor(const unsigned int pointSetIndex,
              const unsigned int neighborPointSetIndex,
              const unsigned int index, const unsigned int k) const {
    return m_neighborhoodSearch->point_set(pointSetIndex)
        .neighbor(neighborPointSetIndex, index, k);
  }

  FORCE_INLINE const unsigned int *
  getNeighborList(const unsigned int pointSetIndex,
                  const unsigned int neighborPointSetIndex,
                  const unsigned int index) const {
#ifdef GPU_NEIGHBORHOOD_SEARCH
    return m_neighborhoodSearch->point_set(pointSetIndex)
        .neighbor_list(neighborPointSetIndex, index);
#else
    return m_neighborhoodSearch->point_set(pointSetIndex)
        .neighbor_list(neighborPointSetIndex, index)
        .data();
