...
#include "SPlisHSPlasH/PBD/TimeStepPBD.h"
...
Simulation::Simulation() {
  ...
  m_boundaryHandlingMethod =
      static_cast<int>(BoundaryHandlingMethods::Bender2019);
}
...
void Simulation::init(const Real particleRadius, const bool sim2D) {
  ...
  setValue(Simulation::GRAD_KERNEL_METHOD,
           Simulation::ENUM_GRADKERNEL_PRECOMPUTED_CUBIC);
...
void Simulation::initParameters() {
  ...
  ENABLE_Z_SORT =
      createBoolParameter("enableZSort", "Enable z-sort", &m_enableZSort);
  setGroup(ENABLE_Z_SORT, "Simulation|Simulation");
  setDescription(ENABLE_Z_SORT, "Enable z-sort to improve cache hits.");

  ParameterBase::GetFunc<Real> getRadiusFct =
      std::bind(&Simulation::getParticleRadius, this);
  ParameterBase::SetFunc<Real> setRadiusFct =
      std::bind(&Simulation::setParticleRadius, this, std::placeholders::_1);
  PARTICLE_RADIUS = createNumericParameter("particleRadius", "Particle radius",
                                           getRadiusFct, setRadiusFct);
  ...
  GRAVITATION = createVectorParameter("gravitation", "Gravitation", 3u,
                                      m_gravitation.data());
  setGroup(GRAVITATION, "Simulation|Simulation");
  setDescription(GRAVITATION,
                 "Vector to define the gravitational acceleration.");
  ...
  EnumParameter *enumParam =
      static_cast<EnumParameter *>(getParameter(CFL_METHOD));
  ...
  CFL_FACTOR =
      createNumericParameter("cflFactor", "CFL - factor", &m_cflFactor);
  ...
  CFL_MIN_TIMESTEPSIZE = createNumericParameter(
      "cflMinTimeStepSize", "CFL - min. time step size", &m_cflMinTimeStepSize);
  setGroup(CFL_MIN_TIMESTEPSIZE, "Simulation|CFL");
  setDescription(CFL_MIN_TIMESTEPSIZE, "Min. time step size.");
  static_cast<RealParameter *>(getParameter(CFL_MIN_TIMESTEPSIZE))
      ->setMinValue(1e-9);

  CFL_MAX_TIMESTEPSIZE = createNumericParameter(
      "cflMaxTimeStepSize", "CFL - max. time step size", &m_cflMaxTimeStepSize);
  setGroup(CFL_MAX_TIMESTEPSIZE, "Simulation|CFL");
  setDescription(CFL_MAX_TIMESTEPSIZE, "Max. time step size.");
  static_cast<RealParameter *>(getParameter(CFL_MAX_TIMESTEPSIZE))
      ->setMinValue(1e-6);

  ParameterBase::GetFunc<int> getKernelFct =
      std::bind(&Simulation::getKernel, this);
  ParameterBase::SetFunc<int> setKernelFct =
      std::bind(&Simulation::setKernel, this, std::placeholders::_1);
  KERNEL_METHOD =
      createEnumParameter("kernel", "Kernel", getKernelFct, setKernelFct);
  ...
  if (!m_sim2D) {
    ...
    enumParam->addEnumValue("Wendland quintic C2",
    ...
    enumParam->addEnumValue("Precomputed cubic spline",
                            ENUM_KERNEL_PRECOMPUTED_CUBIC);
  } else {
    enumParam->addEnumValue("Cubic spline 2D", ENUM_KERNEL_CUBIC_2D);
    enumParam->addEnumValue("Wendland quintic C2 2D",
                            ENUM_KERNEL_WENDLANDQUINTICC2_2D);
  }

  ParameterBase::GetFunc<int> getGradKernelFct =
      std::bind(&Simulation::getGradKernel, this);
  ParameterBase::SetFunc<int> setGradKernelFct =
      std::bind(&Simulation::setGradKernel, this, std::placeholders::_1);
  GRAD_KERNEL_METHOD = createEnumParameter("gradKernel", "Gradient of kernel",
                                           getGradKernelFct, setGradKernelFct);
  setGroup(GRAD_KERNEL_METHOD, "Simulation|Kernel");
  setDescription(GRAD_KERNEL_METHOD,
                 "Gradient of the kernel function used in the SPH model.");
  ...
  ParameterBase::GetFunc<int> getSimulationFct =
      std::bind(&Simulation::getSimulationMethod, this);
  ParameterBase::SetFunc<int> setSimulationFct =
      std::bind(&Simulation::setSimulationMethod, this, std::placeholders::_1);
  SIMULATION_METHOD =
      createEnumParameter("simulationMethod", "Simulation method",
                          getSimulationFct, setSimulationFct);
  ...
  enumParam->addEnumValue("PBD", ENUM_SIMULATION_PBD);
  ...
  BOUNDARY_HANDLING_METHOD =
      createEnumParameter("boundaryHandlingMethod", "Boundary handling method",
                          &m_boundaryHandlingMethod);
  setGroup(BOUNDARY_HANDLING_METHOD, "Simulation|Simulation");
  setDescription(BOUNDARY_HANDLING_METHOD, "Boundary handling method.");
  enumParam =
      static_cast<EnumParameter *>(getParameter(BOUNDARY_HANDLING_METHOD));
...
void Simulation::updateTimeStepSize() {
  if (m_cflMethod == 1)
    ...
    const unsigned int iterations =
        m_timeStep->getValue<unsigned int>(TimeStep::SOLVER_ITERATIONS);
    if (iterations > 10)
...
void Simulation::updateTimeStepSizeCFL() {
  ...
  // fluid particles
  for (unsigned int fluidModelIndex = 0;
       fluidModelIndex < numberOfFluidModels(); fluidModelIndex++) {
  ...
  if (getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012) {
    for (unsigned int i = 0; i < numberOfBoundaryModels(); i++) {
      BoundaryModel_Akinci2012 *bm =
          static_cast<BoundaryModel_Akinci2012 *>(getBoundaryModel(i));
      if (bm->getRigidBodyObject()->isDynamic() ||
          bm->getRigidBodyObject()->isAnimated()) {
  ...
  } else if (sim->getBoundaryHandlingMethod() ==
             BoundaryHandlingMethods::Koschier2017) {
    for (unsigned int boundaryModelIndex = 0;
         boundaryModelIndex < numberOfBoundaryModels(); boundaryModelIndex++) {
      BoundaryModel_Koschier2017 *bm =
          static_cast<BoundaryModel_Koschier2017 *>(
              getBoundaryModel(boundaryModelIndex));
      if (bm->getRigidBodyObject()->isDynamic() ||
          bm->getRigidBodyObject()->isAnimated()) {
    ...
    for (unsigned int boundaryModelIndex = 0;
         boundaryModelIndex < numberOfBoundaryModels(); boundaryModelIndex++) {
      BoundaryModel_Bender2019 *bm = static_cast<BoundaryModel_Bender2019 *>(
          getBoundaryModel(boundaryModelIndex));
      if (bm->getRigidBodyObject()->isDynamic() ||
          bm->getRigidBodyObject()->isAnimated()) {
        maxVel = std::max(maxVel, bm->getMaxVel());
...
void Simulation::setSimulationMethod(const int val) {
  ...
  if ((method < SimulationMethods::WCSPH) ||
      (method >= SimulationMethods::NumSimulationMethods))
  ...
  } else if (method == SimulationMethods::PBD) {
    m_timeStep = new TimeStepPBD();
    m_timeStep->init();
    setValue(Simulation::KERNEL_METHOD, Simulation::ENUM_KERNEL_CUBIC);
    setValue(Simulation::GRAD_KERNEL_METHOD, Simulation::ENUM_GRADKERNEL_CUBIC);
...
void Simulation::setSimulationMethodChangedCallback(
...
void Simulation::emittedParticles(FluidModel *model,
...
void Simulation::addFluidModel(const std::string &id,
  ...
  fm->initModel(id, nFluidParticles, fluidParticles, fluidVelocities,
                fluidObjectIds, nMaxEmitterParticles);
...
void Simulation::updateBoundaryVolume() {
  ...
  // (see Akinci et al. "Versatile rigid - fluid coupling for incompressible
  // SPH", Siggraph 2012
  //////////////////////////////////////////////////////////////////////////
  ...
  for (unsigned int body = 0; body < numberOfBoundaryModels(); body++) {
    if (!getBoundaryModel(body)->getRigidBodyObject()->isDynamic() &&
      ...
      static_cast<BoundaryModel_Akinci2012 *>(getBoundaryModel(body))
  ...
  for (unsigned int i = 0; i < numberOfFluidModels(); i++) {
    ...
    for (unsigned int j = numberOfFluidModels();
         j < m_neighborhoodSearch->point_sets().size(); j++)
      m_neighborhoodSearch->set_active(i, j, true);
  }
}
